...
import { listStalledOrdersSchema, listStalledOrdersHandler, } from './tools/list-stalled-orders.js';
import { retryFulfillmentSchema, retryFulfillmentHandler, } from './tools/retry-fulfillment.js';
...
// ─── Resources ────────────────────────────────────────────────────────────────
server.registerResource('order_details', new ResourceTemplate('order://{order_id}', { list: undefined }), {
    title: 'Order Details',
...
server.registerResource('fulfillment_details', new ResourceTemplate('fulfillment://{order_id}', { list: undefined }), {
...
// ─── Prompts ──────────────────────────────────────────────────────────────────
server.registerPrompt('diagnose_stalled_orders', {
    title: 'Diagnose & Recover Stalled Orders',
    description: 'Guided investigation workflow for discovering stalled processing orders, inspecting fulfillment failure details, and executing safe retries or human escalations.',
    argsSchema: {
        ...
        messages: [
            {
                ...
                content: {
                    type: 'text',
                    text: `Please systematically investigate all stalled fulfillment orders using threshold_hours=${hours}.\n\n` +
...
// ─── Tool registrations ───────────────────────────────────────────────────────
server.tool('get_order', 'Retrieve an order and its associated fulfillment task in a single call. ' +
    'Use this first to diagnose why an order is stuck. Returns null for fulfillment ' +
    'if no task was ever created.', getOrderSchema, getOrderHandler);
server.tool('get_fulfillment', 'Retrieve a fulfillment task by ID, including its full status history, ' +
    'attempt count, and failure reason (if any). Use when you need to drill into ' +
    'why a specific task failed.', getFulfillmentSchema, getFulfillmentHandler);
server.tool('list_stalled_orders', 'List all orders that are paid/processing but have no fulfillment activity. ' +
    ...
    'Use threshold_hours to adjust sensitivity (default: 24 hours).', listStalledOrdersSchema, listStalledOrdersHandler);
server.tool('retry_fulfillment', 'Retry fulfillment for a stalled order. ALWAYS call with confirm=false first to ' +
    ...
    'Includes rate-limiting (10-minute cooldown) to prevent double-retries.', retryFulfillmentSchema, retryFulfillmentHandler);
server.tool('escalate_order', 'Flag an order for human review by writing an audit log entry. ' +
    ...
    'The reason is written permanently to the audit log.', escalateOrderSchema, escalateOrderHandler);
server.tool('get_audit_log', 'Retrieve the complete audit history for an order, sorted chronologically. ' +
    'Shows all retries, escalations, and their timestamps and reasons.', getAuditLogSchema, getAuditLogHandler);
// ─── Express app ──────────────────────────────────────────────────────────────
const app = express();
